...
export const fetchAttemptTests = (profileID) => async (dispatch) => {
  ...
  await fetch(process.env.REACT_APP_API_URL + `/student/attempt-tests/${profileID}`, requestOptions)
...
export const fetchTests = (className) => async (dispatch) => {
  ...
  await fetch(process.env.REACT_APP_API_URL + `/student/tests/${className}`, requestOptions)
    .then((response) => response.json())
    .then((data) => {
      if (data?.error?.name === "TokenExpiredError") {
        dispatch(logoutUser());
...
export const submitTest = (data) => async (dispatch) => {
  ...
  await fetch(process.env.REACT_APP_API_URL + `/student/submit-test/${testID}`, requestOptions)
    .then((response) => response.json())
    .then((data) => {
      if (data) {
        if (data?.error?.name === "TokenExpiredError") {
...
export const fetchTeacherTests = (profileID) => async (dispatch) => {
  ...
  await fetch(process.env.REACT_APP_API_URL + `/teacher/tests/${profileID}`, requestOptions)
    .then((response) => response.json())
    .then((data) => {
      if (data) {
        if (data?.error?.name === "TokenExpiredError") {
